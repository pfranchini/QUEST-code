...
def myfunc(argv):
    ...
    arg_output = ""
    arg_user = ""
    arg_help = "{0} -p <pressure> -d <diameter>".format(argv[0])
...
plt.legend()
plt.savefig('T_vs_DE-'+str(pressure)+'bar.pdf')
plt.show()
...
plt.plot(W,T*1e6,label='WvsT')
plt.title('Temperature vs Resonance width')
#plt.xlim([0, 500])
...
plt.plot(DQ,DW,label='DQvsDW')
plt.title('Width variation vs Deposited energy')
plt.xlim([0, 100e3])
plt.xlabel('$\Delta$Q [eV]')
plt.ylabel('$\Delta(\Delta f_2)$ [Hz]')
plt.show()
...
for t0 in np.arange(50e-6, 310e-6, 50e-6):
    ...
    for dw in np.arange(0,0.5,0.001):  # Delta(Deltaf)
        T2= Temperature_from_Width(W0+dw,pressure)
        T1= Temperature_from_Width(W0,pressure)
        #print(T1,T2,T2-T1)
        DQ = np.append(DQ,(heat_capacity_Cv_B_phase_intergral_from_0(T2, pressure) - heat_capacity_Cv_B_phase_intergral_from_0(T1, pressure)) * volume * 6.242e+18) #[eV]#
...
plt.title('Width variation vs Deposited energy ('+str(pressure)+' bar)')
plt.xlim([0, 100e3])
plt.ylim([0, 0.3])
plt.xlabel('$\Delta$Q [eV]')
plt.ylabel('$\Delta(\Delta f_2)$ [Hz]')
