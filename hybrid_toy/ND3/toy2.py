...
if __name__ == "__main__":
    ...
    plt.hist(
        [df_truth.energy[df_truth.description == 'Cosmics'], df_truth.energy[df_truth.description == 'Source']],
        bins=100,                     # shared bins
        label=['Cosmics', 'Source'],
        color=['orange','green'],
        histtype='step',              # outlined (so they overlay clearly)
        linewidth=1.5
    )
    plt.yscale('log')
    plt.title("Simulated energies")
    ...
    # Create a DF with `time|width|energy|id`
